/**
 * QueryWiz Main Application Frontend
 * Brand: Jadai Studios
...
export default function App() {
  ...
  useEffect(() => {
    if (status === "success" && result && result.sql) {
      ...
      fetchInsights(result.sql, result.rows)
        ...
        .catch(() => setInsightText("Reviewing parameters reveals standard metrics aligned with expected baseline data."))
  ...
  return (
    <div className="min-h-screen bg-[#0a0a0a] text-[#e8e8e8] flex flex-col justify-between selection:bg-[#C9A84CBF] selection:text-[#0a0a0a]">
        ...
        <div className="max-w-7xl mx-auto flex justify-between items-center">
          <div className="flex items-center gap-3">
            <div className="flex flex-col">
              <h1 className="cinzel text-xl sm:text-2xl gold-text font-bold leading-none">QueryWiz</h1>
              <p className="text-[9px] tracking-widest opacity-50 uppercase font-mono">portfolio database metrics</p>
            </div>
          ...
          <div className="flex items-center gap-4">
            <span className="text-[11px] font-mono text-zinc-500 hidden sm:inline jetbrains">Brand: Jadai Studios</span>
            
            {/* COLLAPSIBLE TIMELINE BUTTON */}
            <button
              onClick={() => setIsHistoryOpen((prev) => !prev)}
              className={`flex items-center gap-1.5 text-xs font-mono py-1.5 px-3 rounded gold-border duration-150 cursor-pointer hover:bg-[#111] ${
            ...
            <button
              ...
              className="flex items-center gap-1.5 text-xs font-mono py-1.5 px-3 rounded gold-border text-zinc-400 hover:text-white hover:bg-[#111] duration-150 cursor-pointer bg-[#0c0c0c]"
            >
      ...
      <section className="bg-[#0f0f0f] border-b border-[#C9A84C11] py-4 px-4">
        ...
        <AnimatePresence>
          {isHistoryOpen && (
            <motion.aside
              ...
              className="flex-shrink-0 w-full md:w-[260px] flex flex-col gap-4 border-r md:border-r-0 border-[#C9A84C11] pr-0 md:pr-4 overflow-hidden select-none"
            >
              <div className="flex items-center justify-between border-b border-[#C9A84C11] pb-2">
                <span className="text-xs gold-text uppercase tracking-widest font-mono">Query Timeline</span>
              ...
              <div className="flex-1 overflow-y-auto max-h-[500px] flex flex-col gap-2 scrollbar-none">
                {historyList.length === 0 ? (
                  <div className="text-center py-10 px-4 bg-[#111]/10 rounded border border-[#C9A84C05]">
                  ...
                  historyList.map((item) => (
                    <div
                      ...
                      className="text-left p-2.5 bg-[#111111]/40 hover:bg-[#C9A84C0d] border border-[#C9A84C11] hover:border-[#C9A84C33] rounded duration-150 cursor-pointer flex flex-col gap-1 group active:scale-[0.98] transition-all"
                    >
                      <p className="text-[11px] text-zinc-300 font-medium line-clamp-2 leading-relaxed">
                        "{item.question}"
                      </p>
                      <div className="flex items-center justify-between text-[8.5px] font-mono text-zinc-600">
        ...
        <div className="flex-grow flex flex-col gap-6">
          ...
          <div className="py-2 text-center">
            <motion.h2 
              ...
              className="cinzel text-xl sm:text-2xl gold-text font-bold uppercase tracking-wide"
            >
              Ask Jadai database in plain English
            </motion.h2>
            <p className="text-zinc-500 text-xs mt-1 max-w-lg mx-auto leading-relaxed font-sans">
          ...
          >
            <ExampleChips onSelectChip={handleSelectChip} />
          </motion.div>
          ...
          <main className="flex-grow flex flex-col justify-center min-h-[380px]">
            <AnimatePresence mode="wait">
              ...
              {isLoading && (
                ...
                >
                  ...
                  <p className="text-zinc-600 font-mono text-[11px]">
                    Translating natural text inputs and running database connections.
                  </p>
                </motion.div>
              )}
              ...
              {/* WORKSPACE RESULTS CANVAS (WHEN SUCCEEDED OR IN RETRIED CRASH AND RECOVERED SATE) */}
                ...
                >
                  ...
                  {result?.wasRetried && result?.retrySuccess && (
                    <motion.div 
                      ...
                      className="border border-[#C9A84C33] bg-[#C9A84C0d] p-3 rounded flex items-center gap-3 text-xs text-zinc-300 max-w-4xl mx-auto w-full"
                    >
                      ...
                      <div>
                        ...
                        <span className="ml-1 text-[11px] font-mono">Original SQL errored, but QueryWiz auto-repaired, compiled, and executed corrected query successfully!</span>
                      </div>
                    </motion.div>
                  )}

                  {/* WORKSPACE BODY GRID: LEFT COMPILED SQL / EXPLAINER SECTION, RIGHT GRAPHICAL DISPLAY SECTIONS */}
                  <div className="grid grid-cols-1 md:grid-cols-12 gap-6 items-stretch">
                    
                    {/* LEFT PANEL: SQL DISPLAY CARD AND PATTERN TEACHING INLINE COMPONENT */}
                    <div className="col-span-1 md:col-span-5 flex flex-col gap-4">
                      ...
                      <div className="bg-[#0d0d0d] gold-border rounded p-4 flex flex-col gap-2.5">
                        <div className="flex items-center justify-between border-b border-[#C9A84C11] pb-2">
                          ...
                          <button
                            ...
                            className="text-[10px] font-mono border border-[#C9A84C33] px-2.5 py-0.5 rounded cursor-pointer hover:bg-[#111111] duration-150 uppercase"
                          >
                    ...
                    <div className="col-span-1 md:col-span-7 flex flex-col h-full gap-4">
                      {/* Selection tabs to toggle view */}
                      <div className="flex justify-between items-center bg-[#111]/30 p-1 rounded-lg gold-border">
                        <div className="flex gap-1.5">
                          <button
                            type="button"
                            onClick={() => setActiveTab("table")}
                            className={`flex items-center gap-1 px-3 py-1.5 text-xs font-mono rounded cursor-pointer duration-150 transition-all ${
                          ...
                          {/* Render visualizer toggles if chart constraints pass */}
                          <button
                            type="button"
                  ...
                  {/* AUTONOMOUS AI ANCIENT BOX UPDATES SEGMENT */}
                  <div className="grid grid-cols-1 md:grid-cols-12 gap-6 items-stretch border-t border-[#C9A84C11] pt-6">
                    
                    {/* AUTO METRIC INSIGHT BOX DISPLAY */}
                    <div className="col-span-1 md:col-span-6 bg-[#0c0c0c] gold-border rounded p-4 flex flex-col gap-3">
                      <div className="flex items-center gap-2 font-mono text-[#C9A84C] text-[11px] uppercase tracking-wider font-semibold border-b border-[#C9A84C11] pb-2">
                        <Sparkles className="w-4 h-4 text-[#C9A84C]" />
                        <span>Surprising Auto Insights</span>
                      </div>

                      <div className="text-zinc-300 text-xs leading-relaxed font-mono">
                        {isLoadingInsights ? (
                          <div className="flex items-center gap-2 text-zinc-500">
                    ...
                    {/* FOLLOW-UPclickable ANALYSES CHIPS */}
                    <div className="col-span-1 md:col-span-6 bg-[#0c0c0c] gold-border rounded p-4 flex flex-col gap-3">
                      <div className="flex items-center gap-2 font-mono text-[#C9A84C] text-[11px] uppercase tracking-wider font-semibold border-b border-[#C9A84C11] pb-2">
                      ...
                      <div className="flex flex-col gap-2">
                        {isLoadingFollowups ? (
                          ...
                          <p className="text-[10px] text-zinc-650 font-mono italic">No relevant followups mapped to schema details yet.</p>
                        ) : (
                          <div className="flex flex-col gap-1.5">
                            {followupsList.map((item, idx) => (
                              <button
                                ...
                                className="text-left py-1.5 px-3 bg-[#111] hover:bg-[#C9A84C0d] border border-zinc-900 group hover:border-[#C9A84C55] rounded text-[11px] text-zinc-400 group-hover:text-white duration-150 cursor-pointer active:scale-98 transition-all flex items-center justify-between"
                              >
        ...
        {isAdminOpen && (
          ...
          >
            <motion.div
              ...
              className="bg-[#0b0b0b] gold-border w-full max-w-4xl rounded-lg shadow-2xl overflow-hidden flex flex-col max-h-[90vh]"
            >
              <div className="bg-[#0d0d0d] px-6 py-4 border-b border-[#C9A84C22] flex items-center justify-between">
                ...
                <button
                  ...
                  className="p-1 rounded hover:bg-zinc-900 cursor-pointer text-zinc-400 hover:text-white duration-100"
                >
                  <X className="w-5 h-5" />
                </button>
              </div>
              ...
              <div className="p-6 overflow-y-auto flex-1">
                ...
                {!isAdminAuthorized ? (
                  <form onSubmit={handleAdminVerify} className="max-w-md mx-auto py-12 flex flex-col gap-4">
                    <div className="p-4 bg-[#C9A84C0d] border border-[#C9A84C33] rounded text-xs text-zinc-400 mb-2 leading-relaxed font-mono">
                      <KeyRound className="w-5 h-5 text-[#C9A84C] mb-1.5" />
                      Please enter your password key. Admin console allows re-seeding databases, viewing logs in realtime, and checking connection statuses.
                    </div>
                    {adminError && (
                    ...
                    <button
                      type="submit"
                      className="mt-2 py-2 gold-bg text-black font-bold uppercase text-xs rounded duration-100 cursor-pointer active:scale-95 text-center transition-all shadow font-mono"
                    >
                      Authorize Login
                    </button>
                  </form>
                  ...
                  <div className="flex flex-col gap-6 font-mono">
                    ...
                    <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
                      <div className="bg-[#111] p-3 rounded gold-border select-none">
                      ...
                      <div className="bg-[#111] p-3 rounded gold-border select-none">
                        <p className="text-[9px] text-zinc-500 uppercase">Live DB Mode Configuration</p>
                    ...
                    <div className="p-4 bg-[#111] gold-border rounded flex flex-col sm:flex-row sm:items-center justify-between gap-4 select-none">
                      <div>
                      ...
                      <button
                        ...
                        className="px-4 py-2 bg-red-950/40 text-red-400 border border-red-900 duration-150 rounded text-xs select-none cursor-pointer uppercase hover:bg-[#111]/30 disabled:opacity-50 inline-flex items-center gap-1.5"
                      >
                    ...
                    {/* REAL LIVE AUDITING LOG TABLES */}
                    <div className="flex flex-col gap-2 flex-grow">
                      <span className="text-[11px] font-bold text-zinc-400 uppercase tracking-wider">Live Queries Monitor logs (Last 50 lookups)</span>
                      <div className="border border-[#C9A84C11] rounded overflow-hidden max-h-[220px] overflow-y-auto">
                        <table className="w-full text-left text-[11px] border-collapse bg-[#0c0c0c]">
                          <thead className="bg-[#111111] sticky top-0 border-b border-[#C9A84C11] text-[9px] text-[#C9A84C]">
                            <tr>
                          ...
                          <tbody className="divide-y divide-[#C9A84C11] text-zinc-400">
                            {adminLogs.length === 0 ? (
                              <tr>
                                <td colSpan={6} className="p-4 text-center italic text-zinc-650">No lookup transactions recorded inside system logs tables.</td>
                              </tr>
                            ) : (
                              adminLogs.map((log) => (
                                <tr key={log.id} className="hover:bg-zinc-950/80 duration-100">
                                  <td className="p-2 border-r border-[#C9A84C11] jetbrains">{log.id}</td>
                                  <td className="p-2 border-r border-[#C9A84C11] truncate max-w-[120px]" title={log.question}>"{log.question}"</td>
                                  <td className="p-2 border-r border-[#C9A84C11] truncate max-w-[150px] font-mono select-all text-zinc-500" title={log.generated_sql}>{log.generated_sql}</td>
                                  <td className="p-2 border-r border-[#C9A84C11] jetbrains">{log.execution_ms}ms</td>
                                  <td className="p-2 border-r border-[#C9A84C11] jetbrains text-zinc-300">{log.row_count}</td>
                                  <td className="p-2 truncate max-w-[90px] text-[10px]" title={log.ip_hash}>{log.ip_hash}</td>
                                </tr>
                              ))
                            )}
                          </tbody>
      ...
      <footer className="border-t border-[#C9A84C11] py-6 bg-[#0c0c0c] text-center select-none">
        <p className="text-[11px] font-mono text-zinc-600 uppercase tracking-widest leading-loose">
